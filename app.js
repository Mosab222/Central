...
function handleSiteImage(input){
  ...
  const reader=new FileReader();reader.onload=e=>{
    ...
    let img=slot.querySelector('img')||document.createElement('img');
...
function moduleHTML(i,name){
  return '<div class="module-card" id="mcard-'+i+'">'
    +'<div class="module-header">'
      +'<div class="module-title">'+(i+1)+'. '+name+'</div>'
    ...
    +'</div>'
    +'<div class="img-pair">'
      +'<div class="img-slot" id="mslot-'+i+'-0">'
        +'<div class="upload-hint">'+imgIcon+'<span>صورة كبيرة</span></div>'
        +'<button class="rm-btn" onclick="removeModImg('+i+',0,event)">✕</button>'
        +'<div class="size-label">كبيرة</div>'
        +'<input type="file" accept="image/*" onchange="handleModImg('+i+',0,this)">'
      +'</div>'
      +'<div class="img-slot" id="mslot-'+i+'-1">'
        +'<div class="upload-hint">'+imgIcon+'<span>صورة صغيرة</span></div>'
        +'<button class="rm-btn" onclick="removeModImg('+i+',1,event)">✕</button>'
        +'<div class="size-label">صغيرة</div>'
        +'<input type="file" accept="image/*" onchange="handleModImg('+i+',1,this)">'
      +'</div>'
    +'</div>'
    ...
    +'<button class="btn-add-page" onclick="addExtraPage('+i+')">＋ إضافة صفحة صور لـ "'+name+'"</button>'
    +'</div>';
}

function handleModImg(mIdx,imgIdx,input){
  const file=input.files[0];if(!file)return;
  moduleImages[mIdx][imgIdx]=file;
...
function addExtraPage(mIdx){
  const pi=extraPages[mIdx].length;
  extraPages[mIdx].push({imgs:[null,null,null]});
  const c=document.getElementById('extra-'+mIdx);
  const div=document.createElement('div');
  div.className='extra-page';div.id='ep-'+mIdx+'-'+pi;
  div.innerHTML='<button class="remove-extra-page" onclick="removeExtraPage('+mIdx+','+pi+')">✕</button>'
    ...
    +[0,1,2].map(ph=>
      '<div class="photo-slot">'
        +'<div class="photo-img-slot" id="epslot-'+mIdx+'-'+pi+'-'+ph+'">'
          ...
          +'<input type="file" accept="image/*" onchange="handleExtraImg('+mIdx+','+pi+','+ph+',this)">'
        +'</div>'
        +'<input type="text" class="photo-caption" id="epcap-'+mIdx+'-'+pi+'-'+ph+'" placeholder="تعليق..." maxlength="80">'
      +'</div>'
    ).join('')+'</div>';
  c.appendChild(div);
}
...
async function generateReport(){
  ...
  try{
    ...
    const preparedBy=val('preparedBy')||'الحسن الشريف';
    const jobTitle=val('jobTitle')||'مشرف إدارة المرافق بمنطقة الرياض';
    const regionText=val('region')||'المنطقة الوسطى – الأبواب النجدية';
    const regionCity=val('regionCity')||'بمنطقة الرياض';
    const visitDate=val('visitDate')||'01/01/2025';
    const visitDay=val('visitDay')||'الأحد';
    ...
    s1=s1.replace(/الحسن الشريف/g,preparedBy)
         .replace(/مشرف إدارة المرافق بمنطقة الرياض/g,jobTitle)
         .replace(/المنطقة الوسطى – الأبواب النجدية/g,regionText)
         .replace(/بمنطقة الرياض/g,regionCity)
         .replace(/يوم الاحد الموافق /g,'يوم '+visitDay+' الموافق ');
    s1=patchDate(s1,visitDate);
    ...
    if(siteImage){
      const{b64}=await toSquareJpeg(siteImage);
      zip.file('ppt/media/site_photo.jpeg',b64,{base64:true});
      rels2=addRel(rels2,'rIdSITE','site_photo.jpeg');
      s2=injectShape(s2,'مستطيل 9','rIdSITE');
    }
    zip.file('ppt/slides/slide2.xml',s2);
    zip.file('ppt/slides/_rels/slide2.xml.rels',rels2);

    // Slides 3-8 (module rIds: rId5-rId10)
    for(let i=0;i<6;i++){
      const sn=i+3;
      ...
      if(moduleImages[i][0]){
        const{b64}=await toSquareJpeg(moduleImages[i][0]);
        const nm='mod'+sn+'b.jpeg';
        zip.file('ppt/media/'+nm,b64,{base64:true});
        rels=addRel(rels,'rIdB'+sn,nm);
        slide=injectShape(slide,'مستطيل 3','rIdB'+sn);
      }
      if(moduleImages[i][1]){
        const{b64}=await toSquareJpeg(moduleImages[i][1]);
        const nm='mod'+sn+'s.jpeg';
        zip.file('ppt/media/'+nm,b64,{base64:true});
        rels=addRel(rels,'rIdS'+sn,nm);
        slide=injectShape(slide,'مستطيل 21','rIdS'+sn);
      }
      zip.file('ppt/slides/_rels/slide'+sn+'.xml.rels',rels);
      zip.file('ppt/slides/slide'+sn+'.xml',slide);
    }

    // Extra photo pages
    // module rIds in Wasata: slide3=rId5, slide4=rId6...slide8=rId10
    ...
    const moduleRids=['rId5','rId6','rId7','rId8','rId9','rId10'];
    ...
    for(let mIdx=0;mIdx<6;mIdx++){
      ...
      for(let pi=0;pi<pages.length;pi++){
        ...
        for(let ph=0;ph<3;ph++){
          if(page.imgs[ph]){
            const{b64}=await toSquareJpeg(page.imgs[ph]);
            const nm='ep'+mIdx+'_'+slideNum+'_'+ph+'.jpeg';
            zip.file('ppt/media/'+nm,b64,{base64:true});
            nr=addRel(nr,'rIdEP'+slideNum+ph,nm);
            ns=injectShape(ns,rectNames[ph],'rIdEP'+slideNum+ph);
          }
        }
        ...
        presRels=presRels.replace('</Relationships>',
          '  <Relationship Id="rIdNS'+slideNum+'" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slide" Target="slides/slide'+slideNum+'.xml"/>\n</Relationships>');
        slideNum++;
      }
    }
    ...
    a.href=url;
    a.download='تقرير_وسطى_'+(val('siteName')||'زيارة')+'_'+((val('visitDate')||'').replace(/\//g,'-'))+'.pptx';
...
function patchDate(xml,d){
  const[dd,mm,yyyy]=(d||'01/01/2025').split('/');
  return xml.replace(/2025\/01\/01م/g,yyyy+'/'+mm+'/'+dd+'م')
            .replace(/\/2025 م/g,'/'+yyyy+' م')
            .replace(/202\b/g,yyyy.substring(0,3))
            .replace(/\/01\/01\b/g,'/'+mm+'/'+dd);
}
...
function patchPhotoCaptions(xml,caps){
  let i=0;
  return xml.replace(/<a:t>إضافة تعليق<\/a:t>/g,()=>'<a:t>'+ex(caps[i++]||'')+'</a:t>');
}
